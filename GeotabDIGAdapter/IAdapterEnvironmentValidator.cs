...
namespace GeotabDIGAdapter
{
    ...
    public interface IAdapterEnvironmentValidator<T> where T : IDbOServiceTracking
    {
        /// <summary>
        ...
        void ValidateAdapterMachineName(IAdapterEnvironment<T> adapterEnvironment, List<T> dbOServiceTrackings, DIGAdapterService adapterService);

        /// <summary>
        /// Validates the <see cref="IAdapterEnvironment{T}.AdapterVersion"/> against the <see cref="IDbOServiceTracking"/> in the <paramref name="dbOServiceTrackings"/> identified by <paramref name="adapterService"/>. Intended to help ensure that the same version of the GeotabDIGAdapter is used on all machines in a distributed deployment scenario in which copies of the GeotabDIGAdapter are installed on multiple machines with different services running on each (in order to distribute load and maximize throughput).
        /// </summary>
        /// <param name="adapterEnvironment">The <see cref="IAdapterEnvironment{T}"/> to be validated.</param>
        /// <param name="dbOServiceTrackings">A list of <see cref="IDbOServiceTracking"/> objects to validate the <paramref name="adapterEnvironment"/> against.</param>
        /// <param name="adapterService">The specific <see cref="DIGAdapterService"/> in the <paramref name="dbOServiceTrackings"/> to be validated against.</param>
        void ValidateAdapterVersion(IAdapterEnvironment<T> adapterEnvironment, List<T> dbOServiceTrackings, DIGAdapterService adapterService);
    }
}