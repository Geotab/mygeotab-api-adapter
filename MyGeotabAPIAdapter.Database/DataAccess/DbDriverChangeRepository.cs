...
namespace MyGeotabAPIAdapter.Database.DataAccess
{
    ...
    /// </summary>
    public class DbDriverChangeRepository : BaseRepository<DbDriverChange>
    {
        /// <summary>
        ...
        /// <returns></returns>
        public async Task<long> InsertAsync(ConnectionInfo connectionInfo, List<DbDriverChange> dbDriverChanges, DbConfigFeedVersion dbConfigFeedVersion, CancellationTokenSource cancellationTokenSource, int commandTimeout)
        {
            ...
            try
            {
                using (var connection = await new ConnectionProvider(connectionInfo).GetOpenConnectionAsync())
                {
                    using (var transaction = await connection.BeginTransactionAsync())
                    {
                        ...
                        {
                            await InsertAsync(connection, transaction, dbDriverChange, commandTimeout);
                            insertedRowsCount += 1;
