...
namespace MyGeotabAPIAdapter.Database.Caches
{
    ...
    public interface IGenericGeotabGUIDCacheableDbObjectCache2<T1, T2> where T1 : IGeotabGUIDCacheableDbEntity where T2 : IDatabaseUnitOfWorkContext
    {
        /// <summary>
        ...
        /// <returns></returns>
        Task<T1?> GetObjectAsync(long id);

        /// <summary>
        /// Retrieves an object from the cache. If not found, returns <c>null</c>.
        /// </summary>
        /// <param name="geotabGUIDstring">The <see cref="IGeotabGUIDCacheableDbEntity.GeotabGUIDString"/> of the object to retrieve from the cache.</param>
        /// <returns></returns>
        Task<T1?> GetObjectByGeotabGUIDAsync(string geotabGUIDstring);
        ...
        /// <returns></returns>
        Task<long?> GetObjectIdByGeotabGUIDAsync(Guid geotabGUID);

        /// <summary>
        /// Retrieves the <see cref="IGeotabGUIDCacheableDbEntity.id"/> of an object in the cache. If not found, returns <c>null</c>.
        /// </summary>
        /// <param name="geotabGUIDString">The <see cref="IGeotabGUIDCacheableDbEntity.GeotabGUIDString"/> of the object in the cache for which to return the <see cref="IGeotabGUIDCacheableDbEntity.id"/>.</param>
        /// <returns></returns>
        Task<long?> GetObjectIdByGeotabGUIDStringAsync(string geotabGUIDString);

        /// <summary>
        /// Retrieves all objects from the cache. If no objects are found, the returned list will be empty.
        /// </summary>
        /// <returns></returns>
        Task<List<T1>> GetObjectsAsync();
        ...
        /// <returns></returns>
        Task<List<T1>> GetObjectsAsync(DateTime changedSince);

        /// <summary>
        /// Retrieves a list of objects from the cache where the <see cref="IGeotabGUIDCacheableDbEntity.GeotabGUIDString"/> value is equal to <paramref name="geotabGUIDString"/> and the <see cref="IGeotabGUIDCacheableDbEntity.GeotabId"/> value is equal to <paramref name="geotabId"/>. If no objects are found to meet this criterion, the returned list will be empty.
        /// </summary>
        /// <param name="geotabGUIDString">The value of the <see cref="IGeotabGUIDCacheableDbEntity.GeotabGUIDString"/> for any objects to be returned.</param>
        /// <param name="geotabId">The value of the <see cref="IGeotabGUIDCacheableDbEntity.GeotabId"/> for any objects to be returned.</param>
        /// <returns></returns>
        Task<List<T1>> GetObjectsAsync(string geotabGUIDString, string geotabId);
