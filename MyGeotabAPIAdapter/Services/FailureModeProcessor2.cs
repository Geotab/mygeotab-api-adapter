...
{
    /// <summary>
    /// A <see cref="BackgroundService"/> that extracts <see cref="FailureMode"/> objects from a MyGeotab database for in-memory caching. 
    /// </summary>
    class FailureModeProcessor2 : BackgroundService
    {
        ...
        readonly IBackgroundServiceAwaiter<FailureModeProcessor2> awaiter;
        readonly IExceptionHelper exceptionHelper;
        readonly IGenericGeotabObjectCacher<FailureMode> failureModeGeotabObjectCacher;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="FailureModeProcessor2"/> class.
        /// </summary>
        public FailureModeProcessor2(IAdapterConfiguration adapterConfiguration, IAdapterEnvironment<DbOServiceTracking2> adapterEnvironment, IBackgroundServiceAwaiter<FailureModeProcessor2> awaiter, IExceptionHelper exceptionHelper, IMyGeotabAPIHelper myGeotabAPIHelper, IServiceTracker<DbOServiceTracking2> serviceTracker, IStateMachine2<DbMyGeotabVersionInfo2> stateMachine, IGenericGeotabObjectCacher<FailureMode> failureModeGeotabObjectCacher, IGenericDatabaseUnitOfWorkContext<AdapterDatabaseUnitOfWorkContext> adapterContext)
        {
            ...
            this.failureModeGeotabObjectCacher = failureModeGeotabObjectCacher;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            var delayTimeSpan = TimeSpan.FromMinutes(adapterConfiguration.FailureModeCacheUpdateIntervalMinutes);
                    ...
                    using (var cancellationTokenSource = new CancellationTokenSource())
                    {
                        ...
                        await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
                        {
                            using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                            {
                                ...
                                {
                                    // DbOServiceTracking2:
                                    await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.FailureModeProcessor2, DateTime.UtcNow);
        ...
        async Task InitializeOrUpdateCachesAsync(CancellationTokenSource cancellationTokenSource)
        {
            ...
            if (failureModeGeotabObjectCacher.IsInitialized == false)
            {
                dbObjectCacheInitializationAndUpdateTasks.Add(failureModeGeotabObjectCacher.InitializeAsync(cancellationTokenSource, adapterConfiguration.FailureModeCacheIntervalDailyReferenceStartTimeUTC, adapterConfiguration.FailureModeCacheUpdateIntervalMinutes, adapterConfiguration.FailureModeCacheRefreshIntervalMinutes, myGeotabAPIHelper.GetFeedResultLimitDefault, false));
            }
            else
            {
                dbObjectCacheInitializationAndUpdateTasks.Add(failureModeGeotabObjectCacher.UpdateGeotabObjectCacheAsync(cancellationTokenSource));
            }
        ...
        /// <summary>
        /// Starts the current <see cref="FailureModeProcessor2"/> instance.
        /// </summary>
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.FailureModeProcessor2, adapterConfiguration.DisableMachineNameValidation);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
                    try
                    {
                        await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.FailureModeProcessor2, adapterEnvironment.AdapterVersion.ToString(), adapterEnvironment.AdapterMachineName);
                        await adapterUOW.CommitAsync();
                    }
                    catch (Exception ex)
                    {
            ...
            {
                stateMachine.RegisterService(nameof(FailureModeProcessor2), adapterConfiguration.EnableFailureModeCache);
            }

            // Only start this service if it has been configured to be enabled.
            if (adapterConfiguration.UseDataModel2 == true && adapterConfiguration.EnableFailureModeCache == true)
            {
        ...
        /// <summary>
        /// Stops the current <see cref="FailureModeProcessor2"/> instance.
        /// </summary>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/>.</param>
        /// <returns></returns>
        public override Task StopAsync(CancellationToken cancellationToken)
        {
            ...
            if (adapterConfiguration.UseDataModel2 == true)
            {
                stateMachine.RegisterService(nameof(FailureModeProcessor2), false);
            }
