...
namespace MyGeotabAPIAdapter.Services
{
    ...
    class ZoneProcessor : BackgroundService
    {
                ...
                try
                {
                    ...
                    using (var cancellationTokenSource = new CancellationTokenSource())
                    {
                        ...
                        if (zoneGeotabObjectCacher.LastUpdatedTimeUTC > zoneGeotabObjectCacher.LastPropagatedToDatabaseTimeUtc)
                        {
                            ...
                            // Force the DbZone cache to be updated so that the changes are immediately available to other consumers.
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.ZoneProcessor);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
