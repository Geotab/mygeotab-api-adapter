...
namespace MyGeotabAPIAdapter.Services
{
    ...
    class ControllerProcessor : BackgroundService
    {
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.ControllerProcessor, adapterConfiguration.DisableMachineNameValidation);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
            ...
            if (adapterConfiguration.UseDataModel2 == false && adapterConfiguration.EnableControllerCache == true)
            {
