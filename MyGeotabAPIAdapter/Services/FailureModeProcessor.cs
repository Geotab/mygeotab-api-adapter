...
namespace MyGeotabAPIAdapter.Services
{
    ...
    /// </summary>
    class FailureModeProcessor : BackgroundService
    {
                    ...
                    using (var cancellationTokenSource = new CancellationTokenSource())
                    {
                        ...
                        await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
                        {
                            using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                            {
                                ...
                                {
                                    // DbOServiceTracking:
                                    await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.FailureModeProcessor, DateTime.UtcNow);
        ...
        /// <summary>
        /// Starts the current <see cref="FailureModeProcessor"/> instance.
        /// </summary>
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.FailureModeProcessor, adapterConfiguration.DisableMachineNameValidation);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
                    try
                    {
                        await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.FailureModeProcessor, adapterEnvironment.AdapterVersion.ToString(), adapterEnvironment.AdapterMachineName);
                        await adapterUOW.CommitAsync();
                    }
                    catch (Exception ex)
                    {
            ...
            if (adapterConfiguration.UseDataModel2 == false && adapterConfiguration.EnableFailureModeCache == true)
            {
        ...
        /// <summary>
        /// Stops the current <see cref="FailureModeProcessor"/> instance.
        /// </summary>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/>.</param>
        /// <returns></returns>
