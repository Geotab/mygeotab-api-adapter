...
namespace MyGeotabAPIAdapter.Services
{
    ...
    class StatusDataProcessor : BackgroundService
    {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    using (var cancellationTokenSource = new CancellationTokenSource())
                    {
                        ...
                        {
                            statusDataGeotabObjectFeeder.LastFeedVersion = dbOServiceTracking.LastProcessedFeedVersion;
                            statusDataGeotabObjectFeeder.LastFeedRetrievalTimeUtc = DateTime.MinValue;
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.StatusDataProcessor);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
