...
namespace MyGeotabAPIAdapter.Services
{
    ...
    class ZoneTypeProcessor : BackgroundService
    {
                        ...
                        if (zoneTypeGeotabObjectCacher.LastUpdatedTimeUTC > zoneTypeGeotabObjectCacher.LastPropagatedToDatabaseTimeUtc)
                        {
                            ...
                            foreach (var zoneType in zoneTypeGeotabObjectCacher.GeotabObjectCache.Values)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (dbZoneTypesToPersist.Where(dbZoneType => dbZoneType.GeotabId == newDbZoneType.GeotabId).Any())
                                    {
                            ...
                            // Force the DbZoneType cache to be updated so that the changes are immediately available to other consumers.
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.ZoneTypeProcessor);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
