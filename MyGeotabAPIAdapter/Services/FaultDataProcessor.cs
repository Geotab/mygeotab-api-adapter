...
namespace MyGeotabAPIAdapter.Services
{
    ...
    class FaultDataProcessor : BackgroundService
    {
        ...
        /// </summary>
        public FaultDataProcessor(IAdapterConfiguration adapterConfiguration, IAdapterEnvironment<DbOServiceTracking> adapterEnvironment, IExceptionHelper exceptionHelper, IGenericEntityPersister<DbFaultData> dbFaultDataEntityPersister, IGenericGeotabObjectFeeder<FaultData> faultDataGeotabObjectFeeder, IGenericGeotabObjectHydrator<Controller> controllerGeotabObjectHydrator, IGenericGeotabObjectHydrator<FailureMode> failureModeGeotabObjectHydrator, IGeotabDeviceFilterer geotabDeviceFilterer, IGeotabDiagnosticFilterer geotabDiagnosticFilterer, IGeotabFaultDataDbFaultDataObjectMapper geotabFaultDataDbFaultDataObjectMapper, IMyGeotabAPIHelper myGeotabAPIHelper, IPrerequisiteServiceChecker<DbOServiceTracking> prerequisiteServiceChecker, IServiceTracker<DbOServiceTracking> serviceTracker, IStateMachine<DbMyGeotabVersionInfo> stateMachine, IGenericDatabaseUnitOfWorkContext<AdapterDatabaseUnitOfWorkContext> adapterContext)
        {
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.FaultDataProcessor, adapterConfiguration.DisableMachineNameValidation);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
            ...
            if (adapterConfiguration.UseDataModel2 == false && adapterConfiguration.EnableFaultDataFeed == true)
            {
