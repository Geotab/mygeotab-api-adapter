...
namespace MyGeotabAPIAdapter.Services
{
    ...
    /// </summary>
    class TripProcessor : BackgroundService
    {
        ...
        readonly IGenericEntityPersister<DbTrip> dbTripEntityPersister;
        readonly IGenericGeotabObjectFeeder<Trip> tripGeotabObjectFeeder;
        readonly IGeotabDeviceFilterer geotabDeviceFilterer;
        readonly IGeotabTripDbTripObjectMapper geotabTripDbTripObjectMapper;
        ...
        /// </summary>
        public TripProcessor(IAdapterConfiguration adapterConfiguration, IAdapterEnvironment<DbOServiceTracking> adapterEnvironment, IExceptionHelper exceptionHelper, IGenericEntityPersister<DbTrip> dbTripEntityPersister, IGenericGeotabObjectFeeder<Trip> tripGeotabObjectFeeder, IGeotabDeviceFilterer geotabDeviceFilterer, IGeotabTripDbTripObjectMapper geotabTripDbTripObjectMapper, IMyGeotabAPIHelper myGeotabAPIHelper, IPrerequisiteServiceChecker<DbOServiceTracking> prerequisiteServiceChecker, IServiceTracker<DbOServiceTracking> serviceTracker, IStateMachine<DbMyGeotabVersionInfo> stateMachine, IGenericDatabaseUnitOfWorkContext<AdapterDatabaseUnitOfWorkContext> adapterContext)
        {
            this.adapterConfiguration = adapterConfiguration;
            this.adapterEnvironment = adapterEnvironment;
            this.exceptionHelper = exceptionHelper;
            this.dbTripEntityPersister = dbTripEntityPersister;
            this.tripGeotabObjectFeeder = tripGeotabObjectFeeder;
            this.geotabDeviceFilterer = geotabDeviceFilterer;
            this.geotabTripDbTripObjectMapper = geotabTripDbTripObjectMapper;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    {
                        var dbOServiceTracking = await serviceTracker.GetTripServiceInfoAsync();
                        ...
                        if (trips.Any())
                        {
                            ...
                            // Map the Trip objects to DbTrip entities.
                            dbTripsToPersist = geotabTripDbTripObjectMapper.CreateEntities(filteredTrips);
                            ...
                            using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                            {
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.TripProcessor, DateTime.UtcNow);
                                    }
        ...
        /// <summary>
        /// Starts the current <see cref="TripProcessor"/> instance.
        /// </summary>
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.TripProcessor, adapterConfiguration.DisableMachineNameValidation);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
                    try
                    {
                        await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.TripProcessor, adapterEnvironment.AdapterVersion.ToString(), adapterEnvironment.AdapterMachineName);
                        await adapterUOW.CommitAsync();
                    }
                    catch (Exception ex)
                    {
            ...
            if (adapterConfiguration.UseDataModel2 == false && adapterConfiguration.EnableTripFeed == true)
            {
        ...
        /// <summary>
        /// Stops the current <see cref="TripProcessor"/> instance.
        /// </summary>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/>.</param>
        /// <returns></returns>
