...
namespace MyGeotabAPIAdapter.Services
{
    ...
    class DriverChangeProcessor : BackgroundService
    {
        ...
        readonly IGeotabDriverChangeDbDriverChangeObjectMapper geotabDriverChangeDbDriverChangeObjectMapper;
        ...
        /// </summary>
        public DriverChangeProcessor(IAdapterConfiguration adapterConfiguration, IAdapterEnvironment<DbOServiceTracking> adapterEnvironment, IExceptionHelper exceptionHelper, IGenericEntityPersister<DbDriverChange> dbDriverChangeEntityPersister, IGenericGeotabObjectFeeder<DriverChange> driverChangeGeotabObjectFeeder, IGeotabDeviceFilterer geotabDeviceFilterer, IGeotabDriverChangeDbDriverChangeObjectMapper geotabDriverChangeDbDriverChangeObjectMapper, IMyGeotabAPIHelper myGeotabAPIHelper, IPrerequisiteServiceChecker<DbOServiceTracking> prerequisiteServiceChecker, IServiceTracker<DbOServiceTracking> serviceTracker, IStateMachine<DbMyGeotabVersionInfo> stateMachine, IGenericDatabaseUnitOfWorkContext<AdapterDatabaseUnitOfWorkContext> adapterContext)
        {
            ...
            this.geotabDriverChangeDbDriverChangeObjectMapper = geotabDriverChangeDbDriverChangeObjectMapper;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    {
                        var dbOServiceTracking = await serviceTracker.GetDriverChangeServiceInfoAsync();
                        ...
                        {
                            driverChangeGeotabObjectFeeder.LastFeedVersion = dbOServiceTracking.LastProcessedFeedVersion;
                            driverChangeGeotabObjectFeeder.LastFeedRetrievalTimeUtc = DateTime.MinValue;
                        ...
                        if (driverChanges.Any())
                        {
                            ...
                            // Map the DriverChange objects to DbDriverChange entities.
                            dbDriverChangesToPersist = geotabDriverChangeDbDriverChangeObjectMapper.CreateEntities(filteredDriverChanges);
                            ...
                            using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                            {
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.DriverChangeProcessor, DateTime.UtcNow);
                                    }
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.DriverChangeProcessor, adapterConfiguration.DisableMachineNameValidation);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
                    try
                    {
                        await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.DriverChangeProcessor, adapterEnvironment.AdapterVersion.ToString(), adapterEnvironment.AdapterMachineName);
                        await adapterUOW.CommitAsync();
                    }
                    catch (Exception ex)
                    {
            ...
            if (adapterConfiguration.UseDataModel2 == false && adapterConfiguration.EnableDriverChangeFeed == true)
            {
