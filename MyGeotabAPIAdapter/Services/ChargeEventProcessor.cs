...
namespace MyGeotabAPIAdapter.Services
{
    ...
    /// </summary>
    class ChargeEventProcessor : BackgroundService
    {
        ...
        readonly IGenericEntityPersister<DbChargeEvent> dbChargeEventEntityPersister;
        ...
        readonly IGeotabChargeEventDbChargeEventObjectMapper geotabChargeEventDbChargeEventObjectMapper;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ChargeEventProcessor"/> class.
        /// </summary>
        public ChargeEventProcessor(IAdapterConfiguration adapterConfiguration, IAdapterEnvironment<DbOServiceTracking> adapterEnvironment, IExceptionHelper exceptionHelper, IGenericEntityPersister<DbChargeEvent> dbChargeEventEntityPersister, IGenericGeotabObjectFeeder<ChargeEvent> chargeEventGeotabObjectFeeder, IGeotabDeviceFilterer geotabDeviceFilterer, IGeotabChargeEventDbChargeEventObjectMapper geotabChargeEventDbChargeEventObjectMapper, IMyGeotabAPIHelper myGeotabAPIHelper, IPrerequisiteServiceChecker<DbOServiceTracking> prerequisiteServiceChecker, IServiceTracker<DbOServiceTracking> serviceTracker, IStateMachine<DbMyGeotabVersionInfo> stateMachine, IGenericDatabaseUnitOfWorkContext<AdapterDatabaseUnitOfWorkContext> adapterContext)
        {
            this.adapterConfiguration = adapterConfiguration;
            this.adapterEnvironment = adapterEnvironment;
            this.exceptionHelper = exceptionHelper;
            this.dbChargeEventEntityPersister = dbChargeEventEntityPersister;
            this.chargeEventGeotabObjectFeeder = chargeEventGeotabObjectFeeder;
            this.geotabDeviceFilterer = geotabDeviceFilterer;
            this.geotabChargeEventDbChargeEventObjectMapper = geotabChargeEventDbChargeEventObjectMapper;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    {
                        var dbOServiceTracking = await serviceTracker.GetChargeEventServiceInfoAsync();

                        // Initialize the Geotab object feeder.
                        if (chargeEventGeotabObjectFeeder.IsInitialized == false)
                        {
                            await chargeEventGeotabObjectFeeder.InitializeAsync(cancellationTokenSource, adapterConfiguration.ChargeEventFeedIntervalSeconds, myGeotabAPIHelper.GetFeedResultLimitDefault, dbOServiceTracking.LastProcessedFeedVersion);
                        }
                        ...
                        if (chargeEvents.Any())
                        {
                            ...
                            // Map the ChargeEvent objects to DbChargeEvent entities.
                            dbChargeEventsToPersist = geotabChargeEventDbChargeEventObjectMapper.CreateEntities(filteredChargeEvents);
                            ...
                            using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                            {
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.ChargeEventProcessor, DateTime.UtcNow);
                                    }
        ...
        /// <summary>
        /// Starts the current <see cref="ChargeEventProcessor"/> instance.
        /// </summary>
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            adapterEnvironment.ValidateAdapterEnvironment(dbOserviceTrackings, AdapterService.ChargeEventProcessor, adapterConfiguration.DisableMachineNameValidation);
            await asyncRetryPolicyForDatabaseTransactions.ExecuteAsync(async pollyContext =>
            {
                using (var adapterUOW = adapterContext.CreateUnitOfWork(Databases.AdapterDatabase))
                {
                    try
                    {
                        await serviceTracker.UpdateDbOServiceTrackingRecordAsync(adapterContext, AdapterService.ChargeEventProcessor, adapterEnvironment.AdapterVersion.ToString(), adapterEnvironment.AdapterMachineName);
                        await adapterUOW.CommitAsync();
                    }
                    catch (Exception ex)
                    {
            ...
            if (adapterConfiguration.UseDataModel2 == false && adapterConfiguration.EnableChargeEventFeed == true)
            {
        ...
        /// <summary>
        /// Stops the current <see cref="ChargeEventProcessor"/> instance.
        /// </summary>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/>.</param>
        /// <returns></returns>
