...
--   from version 3.14.0.0 to version 4.0.1.0.
--
-- NOTES: 
...
INSERT INTO #TMP_UpgradeDatabaseVersionTable VALUES ('4.0.1.0');

DECLARE @requiredStartingDatabaseVersion NVARCHAR(50) = '3.14.0.0';
...
BEGIN
	...
	BEGIN TRY
		...
		-- Validate that @filePath exists (only on first run before any FileGroups are created).
		-- Once FileGroups exist, the path was already validated by SQL Server when creating the .ndf files.
		IF NOT EXISTS (SELECT 1 FROM sys.filegroups WHERE name LIKE @fileGroupNamePrefix + '%')
		BEGIN
			DECLARE @fileExistsResults TABLE (
				FileExists INT,
				IsDirectory INT,
				ParentDirectoryExists INT
			);
			INSERT INTO @fileExistsResults
			EXEC master.dbo.xp_fileexist @filePath;
			
			IF NOT EXISTS (SELECT 1 FROM @fileExistsResults WHERE IsDirectory = 1)
			BEGIN
				RAISERROR('ERROR: The specified @filePath does not exist: ''%s''. Please update the @filePath variable to a valid directory path.', 16, 1, @filePath);
				RETURN;
			END
		END
		...
		-- Adjust @currentDateUTC based on the @PartitionInterval:
		IF @PartitionInterval = 'monthly'
			-- Reset @currentDateUTC to the first day of the starting month for monthly partitioning.
			SET @currentDateUTC = DATEFROMPARTS(YEAR(@minDateUTC), MONTH(@minDateUTC), 1);
		ELSE IF @PartitionInterval = 'weekly'
			-- Reset @currentDateUTC to the first day of the week for weekly partitioning (ISO 8601 weeks start on Monday).
			SET @currentDateUTC = DATEADD(DAY, 1 - DATEPART(WEEKDAY, @minDateUTC), @minDateUTC);
		ELSE
			-- No @currentDateUTC adjustment needed for daily partitioning.
			SET @currentDateUTC = @minDateUTC;
