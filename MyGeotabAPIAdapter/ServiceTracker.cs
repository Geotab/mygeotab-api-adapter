...
namespace MyGeotabAPIAdapter
{
    ...
    public class ServiceTracker<T> : IServiceTracker<T> where T : class, IDbOServiceTracking, new()
    {
        ...
        /// <inheritdoc/>
        public async Task<T> GetDutyStatusLogService2InfoAsync()
        {
            await ReloadDbOServiceTrackingObjectCacheIfStaleAsync();
            var dbOServiceTracking = await dbOServiceTrackingObjectCache.GetObjectAsync(AdapterService.DutyStatusLogProcessor2.ToString());
            return dbOServiceTracking;
        }
        /// <inheritdoc/>
        ...
        /// <inheritdoc/>
        public async Task<T> GetShipmentLogService2InfoAsync()
        {
            await ReloadDbOServiceTrackingObjectCacheIfStaleAsync();
            var dbOServiceTracking = await dbOServiceTrackingObjectCache.GetObjectAsync(AdapterService.ShipmentLogProcessor2.ToString());
            return dbOServiceTracking;
        }
