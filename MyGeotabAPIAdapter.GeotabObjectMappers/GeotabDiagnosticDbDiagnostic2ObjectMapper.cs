...
{
    /// <summary>
    /// A class with methods involving mapping between <see cref="Diagnostic"/> and <see cref="DbDiagnostic2"/> entities.
    /// </summary>
    public class GeotabDiagnosticDbDiagnostic2ObjectMapper : IGeotabDiagnosticDbDiagnostic2ObjectMapper
    {
        ...
        public GeotabDiagnosticDbDiagnostic2ObjectMapper(IStringHelper stringHelper)
        {
            this.stringHelper = stringHelper;
        }

        /// <inheritdoc/>
        public DbDiagnostic2 CreateEntity(Diagnostic entityToMapTo, Common.DatabaseRecordStatus entityStatus = Common.DatabaseRecordStatus.Active)
        {
            ...
            DbDiagnostic2 dbDiagnostic2 = new()
            {
            ...
            {
                dbDiagnostic2.ControllerId = diagnosticController.Id.ToString();

                // Derive the OBD-II Diagnostic Trouble Code (DTC), if applicable.
                if (dbDiagnostic2.DiagnosticSourceId == KnownId.SourceObdId.ToString() || dbDiagnostic2.DiagnosticSourceId == KnownId.SourceObdSaId.ToString())
                {
                    int diagnosticCode;
                    if (entityToMapTo.Code != null)
                    {
                        ...
                        if (dtcPrefix.Length > 0)
                        {
                            ...
                            dbDiagnostic2.OBD2DTC = $"{dtcPrefix}{dtc.ToUpper()}";
                        }
                    }
                }
            }
            return dbDiagnostic2;
        }
        ...
        /// <returns></returns>
        static bool DiagnosticIdHasChanged(DbDiagnostic2 dbDiagnostic, Diagnostic diagnostic)
        {
        ...
        /// <inheritdoc/>
        public bool EntityRequiresUpdate(DbDiagnostic2 entityToEvaluate, Diagnostic entityToMapTo)
        {
            if (entityToEvaluate.GeotabId != entityToMapTo.Id.ToString())
            {
                ...
                if (diagnosticIdHasChanged == false)
                {
                    throw new ArgumentException($"Cannot compare {nameof(DbDiagnostic2)} '{entityToEvaluate.id}' with {nameof(Diagnostic)} '{entityToMapTo.Id}' because the IDs do not match.");
                }
            }
        ...
        /// <inheritdoc/>
        public DbDiagnostic2 UpdateEntity(DbDiagnostic2 entityToUpdate, Diagnostic entityToMapTo, Common.DatabaseRecordStatus entityStatus = Common.DatabaseRecordStatus.Active)
        {
            if (entityToUpdate.GeotabId != entityToMapTo.Id.ToString())
            {
                throw new ArgumentException($"Cannot update {nameof(DbDiagnostic2)} '{entityToUpdate.id} (GeotabId {entityToUpdate.GeotabId})' with {nameof(Diagnostic)} '{entityToMapTo.Id}' because the GeotabIds do not match.");
            }
